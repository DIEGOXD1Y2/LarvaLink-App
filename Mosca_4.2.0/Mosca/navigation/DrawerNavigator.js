...
export default function DrawerNavigator({ userData, onLogout }) {
  ...
  useEffect(() => {
    ...
    const fetchAlertas = async () => {
      try {
        ...
        const res = await axios.get(`http://10.165.146.235:3000/alertas/fecha?fecha=${fechaHoy}`);
        const data = res.data;
  ...
  return (
    <Drawer.Navigator
      ...
      screenOptions={({ route }) => ({
        drawerActiveTintColor: '#246928ff',    // verde fuerte para seleccionado
        drawerInactiveTintColor: '#31cc39ff',  // verde claro para no seleccionado
